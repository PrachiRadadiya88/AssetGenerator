...
from app.prompts.prompts import GENERATE_VIDEO_VISION_PROMPT
...
async def generate_video(
    ...
    """
    Generate a Play Store promotional video asset using the multi-agent pipeline.
    
    Accepts app details, video type, user description, and optional screenshot uploads.
    Returns the full pipeline output: creative brief, storyboard, scene visuals,
    compliance report, editing spec, and generated scene frame images.
    """
    ...
    # Parse features
    try:
        feature_list = json_module.loads(features)
    ...
    if screenshots:
        for i, file in enumerate(screenshots):
            if file.filename and file.size > 0:
                ext = os.path.splitext(file.filename)[1] or ".png"
                save_path = os.path.join(session_dir, f"video_upload_{i}{ext}")
                content = await file.read()
                with open(save_path, "wb") as f:
                    f.write(content)
                screenshot_paths.append(save_path)
                logger.info(f"Saved video screenshot upload: {save_path}")
    
    # Save logo if provided
    logo_path = None
    if logo and logo.filename and logo.size > 0:
        ext = os.path.splitext(logo.filename)[1] or ".png"
        logo_path = os.path.join(session_dir, f"app_logo{ext}")
        content = await logo.read()
        with open(logo_path, "wb") as f:
            f.write(content)
        logger.info(f"Saved app logo: {logo_path}")
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail=f"Video generation timed out: {str(e)}")
    except Exception as e:
